...
object AudienceParquetTransformer {
  def main(args: Array[String]) {
    ...
    var inputPath = "D:\\sparkTestFile\\"
    //var outputPath = "/tmp/output"
    var outputPath = "D:\\parquet"
    ...
    val conf = new SparkConf().setMaster(masterUrl).setAppName("AudienceParquetTransformer")
     ...
     * Step 1: Prepare RDDs
     */
//    val DATA_PATH =  "hdfs://192.168.1.160:8020/home"
    ...
    peopleDF.write.format("parquet").mode("overwrite").save(outputPath)

    val txtDF = peopleDF
    txtDF.registerTempTable("txtTable")

    val parquetDF = sqlContext.read.parquet(outputPath)
